...
public class QuartzManager {
	...
	private static String JOB_GROUP_NAME = "QUARTZ_JOBGROUP_NAME"; // 任务组
	private static String TRIGGER_GROUP_NAME = "QUARTZ_TRIGGERGROUP_NAME"; // 触发器组

	/**
	 * 描述：添加一个定时任务，使用默认的任务组名，触发器名，触发器组名  
	 * @param jobName 任务名称
	 * @param jobClass 任务class
	 * @param time  运行时间  规则参考quartz文档
	 * @return
	 */
	public static void addJob(String jobName, Class<? extends Job> jobClass, String time) {
		try {
			...
			JobDetail jobDetail = JobBuilder.newJob(jobClass).withIdentity(jobName, JOB_GROUP_NAME).build(); // 用于描叙Job实现类及其他的一些静态信息，构建一个作业实例
			...
			logger.error("Job {} run error!{}", jobClass, e);
		}
	}

	/**
	 * 
	 * 描述：添加一个定时任务，使用默认的任务组名，触发器名，触发器组名  （带参数）
	 * @param jobName 任务名称
	 * @param jobClass 任务class
	 * @param time  运行时间  规则参考quartz文档
	 * @param parameter  参数
	...
	public static void modifyJobTime(String jobName, String time) {
		try {
			...
			logger.error("ModifyJobTime {} run error!{}", jobName, e);
		}
	}
	...
	public static void modifyJobTime(String triggerName, String triggerGroupName, String time) {
		try {
			...
			logger.error("ModifyJobTime {} run error!{}", triggerName, e);
		}
	}
	...
	public static void removeJob(String jobName) {
		try {
			...
			logger.error("RemoveJob {} run error!{}", jobName, e);
		}
	}
	...
	public static void startJobs() {
		try {
			...
			logger.error("Job startJobs error!{}", e);
		}
	}
	...
	public static void shutdownJobs() {
		try {
			...
			logger.error("Job shutdownJobs error!{}", e);
		}
	}
}